...
#pragma warning restore RS2008 // Enable analyzer release tracking
        ...
        private void AnalyzeMethodDeclaration(SymbolAnalysisContext context)
        {
            try
            {
                if (context.Symbol is not IMethodSymbol methodSymbol || !IsActionCandidate(methodSymbol))
                {
                    return;
                }
                ...
                var allPublicMethods = methodSymbol.ContainingType
                    .GetMembers()
                    .OfType<IMethodSymbol>()
                    .Where(IsActionCandidate)
                    .OrderBy(s => s.Locations.First().GetLineSpan().StartLinePosition)
                    .ToList();

                // if our methodSymbol is the first method, then don't display error
                if (allPublicMethods.First().Equals(methodSymbol, SymbolEqualityComparer.Default)) return;
                ...
                static bool IsActionCandidate(IMethodSymbol m)
                {
                    return !m.IsStatic
